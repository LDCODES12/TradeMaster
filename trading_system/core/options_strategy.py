...
class PrecisionOptionsArbitrage:
    ...
    def _get_option_contracts(self, symbol: str) -> List[Dict[str, Any]]:
        """Get options chain data with multi-provider support"""
        # Check if we have an options provider initialized
        if not hasattr(self, 'options_provider'):
            # Create options provider if not already created
            from utils.options_provider import MultiProviderOptionsData
            self.options_provider = MultiProviderOptionsData({
                'alpaca': {
                ...
                'provider_preference': ['polygon', 'alpaca']
            })

        # Get options from the provider
        return self.options_provider.get_option_chain(symbol)
